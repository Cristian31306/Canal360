...
const faqs = [
    {
        ...
        respuesta: 'Nuestro compromiso es la rapidez. Recibirás una respuesta inicial en menos de 15 minutos a través de WhatsApp o correo electrónico.'
    },
    {
        pregunta: '¿Qué tipo de seguros manejan?',
        respuesta: 'Manejamos un ecosistema integral: desde protección para personas (Vida, Salud) y activos (Auto, Hogar) hasta pólizas especializadas para sectores industriales y mineros.'
    },
    {
        pregunta: '¿Con qué aseguradoras trabajan?',
        respuesta: 'Trabajamos con las aseguradoras más sólidas de Colombia (Sura, Allianz, AXA Colpatria, entre otras) para garantizarte siempre el mejor respaldo.'
    },
    {
        pregunta: '¿Cómo es el proceso de reclamación?',
        respuesta: 'Es directo y acompañado. Solo necesitas contactarnos y un asesor experto te guiará paso a paso hasta la resolución del siniestro.'
    }
];
...
<template>
    ...
    <div class="min-h-screen bg-white text-slate-900 font-sans overflow-x-hidden relative">
        ...
        <a :href="'https://wa.me/' + settings.landing_whatsapp_number?.replace(/\s+/g, '')" target="_blank"
            class="fixed bottom-8 right-8 z-50 group flex flex-col items-end gap-3">

            <!-- Tooltip / Greeting Badge -->
            <div
                class="bg-white px-4 py-2 rounded-xl shadow-lg border border-slate-100 text-slate-700 text-sm font-bold opacity-0 group-hover:opacity-100 transition-all duration-300 translate-y-2 group-hover:translate-y-0 pointer-events-none whitespace-nowrap hidden md:block">
            ...
            <div class="relative">
                ...
                <!-- Main Button -->
                <div
                    class="relative w-16 h-16 bg-[#25D366] text-white rounded-full shadow-2xl shadow-[#25D366]/30 flex items-center justify-center hover:bg-[#20ba5c] transition-all hover:scale-110 active:scale-95 group-hover:shadow-[#25D366]/50">
