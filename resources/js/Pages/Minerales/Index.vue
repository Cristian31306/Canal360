    ...
    <AuthenticatedLayout>
        ...
        <div class="py-6">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <!-- Filtros -->
                <div class="mb-8 p-4 bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm flex items-center gap-4">
                    <div class="w-48">
                        <label class="text-[10px] font-black text-gray-400 uppercase tracking-widest block mb-1">Filtrar por Año</label>
                    ...
                    <div v-if="precios.data.length > 0" class="overflow-x-auto">
                        <table class="w-full text-left border-collapse">
                            ...
                            <tbody class="divide-y divide-gray-50 dark:divide-gray-700">
                                <tr v-for="precio in precios.data" :key="precio.id" class="hover:bg-amber-50/20 dark:hover:bg-amber-900/10 transition-colors group">
                                    <td class="px-6 py-6">
                                        <div class="flex flex-col">
                                            <span class="text-sm font-black text-gray-900 dark:text-white uppercase tracking-tight">{{ getMesNombre(precio.mes) }}</span>
                                            <span class="text-xs text-amber-600 font-bold tracking-widest">{{ precio.anio }}</span>
                                        </div>
                                    </td>
                                    ...
                                    <td class="px-6 py-6 text-right border-l border-gray-50/50 dark:border-gray-700/50">
                                        <div class="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                            <Link :href="route('minerales.edit', precio.id)" class="p-2 text-gray-400 hover:text-amber-600 transition-colors">
                                            ...
                                            <button @click="deleteRegistro(precio)" class="p-2 text-gray-400 hover:text-red-600 transition-colors">
                                                <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        ...
                        <div class="px-6 py-4 border-t border-gray-100 dark:border-gray-700 bg-gray-50/30 dark:bg-gray-900/30">
                            <Pagination :links="precios.links" />
                        </div>
                    </div>
                    
                    <div v-else class="text-center py-20 flex flex-col items-center">
                        <div class="w-20 h-20 bg-amber-50 dark:bg-amber-900/20 rounded-full flex items-center justify-center mb-4 text-amber-300">
                            <svg class="w-10 h-10" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                            </svg>
                        </div>
                        <h3 class="text-lg font-black text-slate-700 dark:text-white uppercase tracking-tight">Sin registros de precios</h3>
                        <p class="text-slate-400 text-xs font-bold uppercase tracking-widest mt-1">Lleva el control histórico de los metales mes a mes.</p>
                    </div>
                </div>
            </div>
        </div>
